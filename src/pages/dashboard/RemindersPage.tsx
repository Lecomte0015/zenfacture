...
import { FiPlus, FiTrash2, FiEdit2, FiCalendar, FiAlertTriangle, FiBell } from 'react-icons/fi';
...
// Cette page réutilise volontairement le même hook/service que le back-office
// (useAdminReminders / adminReminderService, table `admin_reminders`) : le
// modèle de données était déjà scopé par utilisateur (RLS : auth.uid() =
// user_id, sans aucune vérification de rôle admin), donc chaque client
// ZenFacture peut déjà avoir ses propres rappels sans changement côté base.
// Seul manquait un point d'entrée côté dashboard client — c'est cette page.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingReminder) {
        ...
        toast({ title: 'Rappel mis à jour', description: 'Le rappel a été mis à jour avec succès.' });
      } else {
        await addReminder({
          ...formData,
          due_date: new Date(formData.due_date).toISOString(),
        });
        toast({ title: 'Rappel ajouté', description: 'Le rappel a été ajouté avec succès.' });
      }
  ...
  const handleDeleteReminder = async (id: string) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce rappel ?')) {
      try {
        ...
        toast({ title: 'Rappel supprimé', description: 'Le rappel a été supprimé avec succès.' });
      } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-sm text-gray-500 mb-8">
        Ne manquez plus une échéance : TVA, AVS, LPP, impôts... Ces rappels sont personnels et ne sont visibles que par vous.
      </p>
      ...
      <div className="mb-8">
        ...
        {reminders.length === 0 ? (
          <div className="bg-white rounded-lg shadow border border-gray-200 text-center py-16">
            <FiBell className="mx-auto h-10 w-10 text-gray-300 mb-3" />
            <p className="text-gray-500">Aucun rappel pour le moment</p>
            <p className="text-sm text-gray-400 mt-1">Ajoutez votre prochaine échéance de cotisation ou de TVA</p>
          </div>
        ) : (
                ...
                {[...upcoming, ...others, ...past].map(reminder => (
                  <tr key={reminder.id} className={isBefore(parseISO(reminder.due_date), new Date()) ? 'bg-red-50' : ''}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button onClick={() => handleOpenEditDialog(reminder)} className="text-blue-600 hover:text-blue-900 mr-4">
                        <FiEdit2 className="h-4 w-4" />
                      </button>
                      <button onClick={() => handleDeleteReminder(reminder.id!)} className="text-red-600 hover:text-red-900">
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  placeholder="Ex: Cotisation AVS T3"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setFormData(prev => ({ ...prev, status: value as ReminderStatus }))}
                  >
                    <SelectTrigger>
            ...
            <DialogFooter>
              ...
              <Button type="submit">{editingReminder ? 'Mettre à jour' : 'Ajouter'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
    ...
    <Card className={`${isOverdue ? 'border-red-300' : ''}`}>
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          ...
          <Badge className={STATUS_COLORS[reminder.status]}>{STATUS_LABELS[reminder.status]}</Badge>
        </div>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex space-x-2">
            <Button variant="ghost" size="sm" onClick={() => onEdit(reminder)} className="text-blue-600 hover:bg-blue-50">
              <FiEdit2 className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="sm" onClick={() => onDelete(reminder.id!)} className="text-red-600 hover:bg-red-50">
              <FiTrash2 className="h-4 w-4" />
            </Button>
          </div>
        </div>
